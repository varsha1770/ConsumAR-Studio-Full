...
export async function GET(req: Request) {
  try {
    ...
    if (userId || userEmail) {
      ...
      if (data.isAdmin) {
        return NextResponse.json({
          success: true,
          tier: "SUPER_ADMIN",
          usage: { rescales: 0, maxRescales: 999999, uploads: 0, maxUploads: 999999 }
        });
      }
      ...
      if (guestResults.length > 0) {
        ...
        if (expiresAt < now) {
          ...
          await prisma.$executeRawUnsafe(`
            UPDATE "GuestUsage" SET "rescaleCount" = 0, "uploadCount" = 0, "usdzCount" = 0, 
            "expiresAt" = $1::timestamp, "lastUsage" = NOW() WHERE "id" = $2::uuid
          `, nextExpiry.toISOString(), guest.id);

          rescalesUsed = 0; usdzUsed = 0; uploadsUsed = 0;
        } else {
    ...
    return NextResponse.json({
      success: true,
      tier: "NON_LOGGED",
      usage: { rescales: 0, maxRescales: currentConfig.dailyRescaleLimit, uploads: 0, maxUploads: 10 }
    });
