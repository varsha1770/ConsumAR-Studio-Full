...
# V145: Only apply watermark if it is a download/finalization
is_download = args.get('is_download', False)
should_watermark = args.get('watermark', False) and is_download

           watermark=should_watermark,
           watermark_text=args.get('watermark_text', 'TryitFirstLabs'),
           remove_watermark=args.get('remove_watermark', False))
...
args   = json.loads(sys.argv[1])
# V145: USDZ is always a final step, so watermark is usually ON
result = convert_s3_glb_to_usdz(args['s3_key'], watermark=args.get('watermark', True), watermark_text=args.get('watermark_text', 'TryitFirstLabs'))
...
def resize_api():
    ...
    try:
        ...
        # V145: Unified Watermark detection with Download Mode check
        ...
        remove_watermark = data.get("remove_watermark", "false").lower() == "true"
        
        # Watermark is ON if Tier is not PAID (or forced) AND it is a download request
        is_paid = tier.upper() == "PAID"
        is_forced = str(force_watermark).lower() == "true"
        watermark = (not is_paid or is_forced)
        
        logger.info(f"!!! WATERMARK STATUS: tier={tier}, forced={force_watermark}, is_download={is_download} => FINAL={watermark and is_download} !!!")
...
def convert_usdz_api():
    ...
    try:
        ...
        proc, stdout, stderr = _run_child(
            _SCRIPT_CONVERT_USDZ, {"s3_key": s3_key, "watermark": watermark_flag, "watermark_text": watermark_text}, timeout=180
        )
        result, err = _child_result(proc, stdout, stderr, route)
